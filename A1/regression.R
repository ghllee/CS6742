...
for(i in 0:(n-1))
{
  ...
  colnames(data) <- c("Uniqueness", "ScoreDiff", "Helpful")
...
plotCI(values, ui=U, li=L, pch=".", xlab = "Product Index", ylab = "Influence of Uniqueness",
        main = "Influence of Uniqueness on Helpfulness of Reviews for the Top 300 Products",
        xlim = c(1,300),
        ylim = c(-30, 20))
meanConfInt <- t.test(values)$conf.int
abline(h=meanConfInt[[1]], col=2, lty=2, lwd = 3)
abline(h=mean(values), col=2)
abline(h=meanConfInt[[2]], col=2, lty=2, lwd = 3)
